...
from pccl.llang.buffer import Buffer, BufferSlice
from pccl.llang.channels import NamedChannel, Channel, ChannelType

@dataclass
class GpuConfig:
    input: List[BufferSlice] = field(default_factory=list)
    output: List[BufferSlice] = field(default_factory=list)
    channels: List[NamedChannel] = field(default_factory=list)
...
class ChunkRef:
    ...
    def __hash__(self):
        return hash((self.src_rank, self.buffer, self.index, self.size))

...
from pccl.llang.chunk import NamedChunkContext, NamedChunk, _named_chunk_context
...
class AllToAll(Collective):
    ...
    def init_buffers(self):
        ...
        for r in range(self.num_ranks):
            input_buffer = [None] * (self.num_ranks * self.chunk_factor)
            output_buffer = [None] * (self.num_ranks * self.chunk_factor)
            for index in range(self.num_ranks * self.chunk_factor):
    ...
    def check(self, prog):
        ...
        for r in range(self.num_ranks):
            ...
            for ch in range(self.chunk_factor):
                ...
                chunk = output[index]
                if chunk is None:
                    print(f"Rank {r} chunk {index} is incorrect should be ({self.root}, {ch}) given None")
                    correct = False

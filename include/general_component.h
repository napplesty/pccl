...
template<typename T>
concept FuncStream = requires(T& component, int index, int priority) {
  {component.stream(index)} -> std::convertible_to<void*>;
  {component.stream(index, priority)} -> std::convertible_to<void*>;
};

template<typename T>
concept FuncMemory = requires(T& component, size_t size, ComponentTypeFlags flags, void* ptr) {
  {component.alloc(size, flags)} -> std::convertible_to<void*>;
  {component.free(ptr, flags)} -> std::same_as<void>;
};

template<typename T>
concept FuncRegMem = requires(T& component, void *ptr, size_t size, ComponentTypeFlags flags) {

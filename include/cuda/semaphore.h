...
struct Host2DeviceSemaphoreDeviceHandle {
  ...
  PCCL_CUDA_DEVICE_INLINE bool poll() {
    bool signaled = (atomicLoad(inboundSemaphoreId, memoryOrderAcquire) >
  ...
  PCCL_CUDA_DEVICE_INLINE void wait(int64_t max_spin_cnt = 3e9) {
    POLL_MAYBE_JAILBREAK((atomicLoad(inboundSemaphoreId, memoryOrderRelaxed) <
                          *expectedInboundSemaphoreId),
                         max_spin_cnt);
    (*expectedInboundSemaphoreId)++;
  }
...
struct MemoryDevice2DeviceSemaphoreDeviceHandle {
  ...
  PCCL_CUDA_DEVICE_INLINE void wait(int64_t maxSpinCount = 1e9) {
    (*expectedInboundSemaphoreId) += 1;
    POLL_MAYBE_JAILBREAK((atomicLoad(inboundSemaphoreId, memoryOrderAcquire) <
                          (*expectedInboundSemaphoreId)),
                         maxSpinCount);
  }
  ...
  PCCL_CUDA_DEVICE_INLINE void signal() {
    semaphoreIncrement();
    atomicStore(remoteInboundSemaphoreId, semaphoreGetLocal(),
                memoryOrderSeqCst);
  }
  ...
  PCCL_CUDA_DEVICE_INLINE void semaphoreIncrement() {
    *outboundSemaphoreId += 1;
  }

  PCCL_CUDA_DEVICE_INLINE uint64_t semaphoreGetLocal() const {
    return *outboundSemaphoreId;
  }
#endif
...
template <template <typename> typename InboundDeleter,
          template <typename> typename OutboundDeleter>
class BaseSemaphore {
protected:
  std::future<RegisteredMemory> remoteInboundSemaphoreIdsRegMem_;
...
class Host2DeviceSemaphore
    : public BaseSemaphore<GpuDeleter, std::default_delete> {
private:
  std::shared_ptr<Connection> connection_;

public:
  Host2DeviceSemaphore(Communicator &communicator,
...
class Host2HostSemaphore
    : public BaseSemaphore<std::default_delete, std::default_delete> {
public:
  Host2HostSemaphore(Communicator &communicator,
                     std::shared_ptr<Connection> connection);
  std::shared_ptr<Connection> connection();
  void signal();
  bool poll();
...
class MemoryDevice2DeviceSemaphore
    : public BaseSemaphore<GpuDeleter, GpuDeleter> {
public:
  MemoryDevice2DeviceSemaphore(Communicator &communicator,
                               std::shared_ptr<Connection> connection);

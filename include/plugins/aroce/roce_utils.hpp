...
public:
  ...
  void load(const std::string& library_path = "libibverbs.so") {
    ...
    #define LOAD_VERBS_FUNC(name) \
      ...
      PCCL_THROW_IF(!handle_, RuntimeException, "Failed to load function ", #name);
...
private:
  ...
  ibv_get_device_list_fn ibv_get_device_list;
  ibv_free_device_list_fn ibv_free_device_list;
  ibv_get_device_name_fn ibv_get_device_name;
  ibv_open_device_fn ibv_open_device;
  ibv_close_device_fn ibv_close_device;
  ibv_query_device_fn ibv_query_device;
  ibv_query_port_fn ibv_query_port;
  ibv_alloc_pd_fn ibv_alloc_pd;
  ibv_dealloc_pd_fn ibv_dealloc_pd;
  ibv_reg_mr_fn ibv_reg_mr;
  ibv_dereg_mr_fn ibv_dereg_mr;
  ibv_create_cq_fn ibv_create_cq;
  ibv_destroy_cq_fn ibv_destroy_cq;
  ibv_poll_cq_fn ibv_poll_cq;
  ibv_create_qp_fn ibv_create_qp;
  ibv_destroy_qp_fn ibv_destroy_qp;
  ibv_modify_qp_fn ibv_modify_qp;
  ibv_post_send_fn ibv_post_send;
  ibv_post_recv_fn ibv_post_recv;
  ibv_req_notify_cq_fn ibv_req_notify_cq;
...
public:
  ...
  QPId createQP(ConnectionId conn_id, const QPConfig& config) {
    ...
    if (it->second.qps.size() >= it->second.config.max_qp_per_connection) {
      return 0;
    }
  ...
  std::shared_ptr<ibv_qp> getQP(ConnectionId conn_id, QPId qp_id) {
    std::lock_guard<std::mutex> lock(connections_mutex_);
    ...
    return qp_it->second->get();
  }

  std::shared_ptr<ibv_cq> getCQ(ConnectionId conn_id) {
    std::lock_guard<std::mutex> lock(connections_mutex_);
    ...
    return it->second.cq->get();
  }

  std::shared_ptr<ibv_pd> getPD() {
    return pd_->get();
  }
  ...
  RemoteConnectionMetadata getLocalMetadata(ConnectionId conn_id) {
    ...
    ibv_port_attr port_attr;
    if (context_->query_port(it->second.config.port_num, &port_attr) == 0) {

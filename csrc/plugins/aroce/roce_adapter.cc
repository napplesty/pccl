...
bool RoCEAdapter::connect(const Endpoint& self, const Endpoint& peer) {
  try {
    ...
    if (!gid_str.empty()) {
      std::memcpy(&remote_info.gid, gid_str.data(), sizeof(remote_info.gid));
    }
...
uint64_t RoCEAdapter::prepSend(const MemRegion& dst, const MemRegion& src) {
  ...
  if (verbs_manager_->postSend(conn_id_, qp_id_, &wr, &bad_wr)) {
    PCCL_LOG_ERROR("Failed to post RoCE send");
    return 0;
  }
  ...
  while (retries < max_retries) {
    ...
    if (count > 0) {
      if (wc.status == IBV_WC_SUCCESS) {
        std::lock_guard<std::mutex> lock(completion_mutex_);
        for (auto& [id, completed] : completion_map_) {
          if (!completed) {
            completed = true;
          }
        }
...
ibv_send_wr RoCEAdapter::buildWriteWR(const MemRegion& dst, const MemRegion& src) {
  ...
  wr.wr.rdma.rkey = std::stoul(peer_endpoint_.attributes_.at("pccl.roce.rkey"));
...
bool RoCEAdapter::waitForCompletion(uint64_t tx_id) {
  ...
  while (retries < max_retries) {
    ...
    if (count > 0) {
      if (wc.wr_id == tx_id) {
        std::lock_guard<std::mutex> lock(completion_mutex_);
        completion_map_[tx_id] = true;
        return wc.status == IBV_WC_SUCCESS;
      }
    }
    
    std::lock_guard<std::mutex> lock(completion_mutex_);
    if (completion_map_.count(tx_id) && completion_map_[tx_id]) {
      return true;
    }

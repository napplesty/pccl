...
TcpRemoteConnectionMetadata::TcpRemoteConnectionMetadata() : port(0) {}

TcpRemoteConnectionMetadata::TcpRemoteConnectionMetadata(const std::string& ip, uint16_t port) 
  : ip(ip), port(port) {}

void TcpGIDMap::registerGID(const GID& gid, const std::string& ip, uint16_t port) {
  std::lock_guard<std::mutex> lock(mutex_);
  map_[gid] = {ip, port};
}

std::optional<std::pair<std::string, uint16_t>> TcpGIDMap::lookupGID(const GID& gid) const {
  auto it = map_.find(gid);
  if (it != map_.end()) return it->second;
  return std::nullopt;
}

TcpDeviceList::TcpDeviceList() {
  struct ifaddrs* ifaddr;
  if (getifaddrs(&ifaddr) == -1) throw std::runtime_error("getifaddrs failed");
...
int TcpDeviceList::count() const {
  return static_cast<int>(devices_.size());
}

std::string TcpDeviceList::operator[](int index) const {
  if (index < 0 || index >= count()) throw std::runtime_error("Index out of range");
  return devices_[index];
}

TcpContext::TcpContext(const std::string& local_ip) : local_ip_(local_ip) {}

const std::string& TcpContext::getLocalIp() const {
  return local_ip_;
}

TcpMemoryRegion::TcpMemoryRegion(void* addr, size_t length, uint32_t lkey, uint32_t rkey)
  : addr_(addr), length_(length), lkey_(lkey), rkey_(rkey) {}

void* TcpMemoryRegion::getAddr() const {
  return addr_;
}

size_t TcpMemoryRegion::getLength() const {
  return length_;
}

uint32_t TcpMemoryRegion::getLkey() const {
  return lkey_;
}

uint32_t TcpMemoryRegion::getRkey() const {
  return rkey_;
}

std::shared_ptr<TcpMemoryRegion> TcpProtectionDomain::registerMR(void* addr, size_t length) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void TcpProtectionDomain::deregisterMR(const std::shared_ptr<TcpMemoryRegion>& mr) {
  std::lock_guard<std::mutex> lock(mutex_);
  mrs_.erase(mr->getLkey());
  mrs_.erase(mr->getRkey());
}

std::shared_ptr<TcpMemoryRegion> TcpProtectionDomain::lookUpMR(uint32_t key) {
  std::lock_guard<std::mutex> lock(mutex_);
  auto it = mrs_.find(key);
  return it != mrs_.end() ? it->second : nullptr;
}

SendTaskContext::SendTaskContext(const TcpSendWR* wr) {
  if (!wr) return;
...
bool SendTaskContext::is_completed() const {
  return curr_sge_idx >= sg_list.size();
}

std::pair<void*, size_t> SendTaskContext::get_current_data() const {
...
bool SendTaskContext::update_sent(size_t bytes) {
...
RecvTaskContext::RecvTaskContext(const TcpRecvWR* wr) {
  if (!wr) return;
  for (const auto* curr_wr = wr; curr_wr; curr_wr = curr_wr->next) {
    if (!curr_wr->sg_list) continue;
    for (int i = 0; i < curr_wr->num_sge; ++i) {
...
bool RecvTaskContext::is_completed() const {
  return curr_sge_idx >= sg_list.size();
}

std::pair<void*, size_t> RecvTaskContext::get_current_buffer() const {
  if (is_completed()) return {nullptr, 0};
  const auto& sge = sg_list[curr_sge_idx];
  return {static_cast<char*>(sge.addr) + curr_sge_offset, sge.length - curr_sge_offset};
}

bool RecvTaskContext::update_received(size_t bytes) {
  if (is_completed()) return false;
  curr_sge_offset += bytes;
  if (curr_sge_offset >= sg_list[curr_sge_idx].length) {
    curr_sge_idx++;
...
WriteTaskContext::WriteTaskContext(const TcpWriteWR* wr) {
  if (!wr) return;
...
bool WriteTaskContext::is_completed() const {
  return curr_sge_idx >= sg_list.size() && ctrl_sent;
}

std::vector<char> WriteTaskContext::get_ctrl_header(const Token& token) const {
...
std::pair<void*, size_t> WriteTaskContext::get_current_data() const {
  if (is_completed()) return {nullptr, 0};
  const auto& sge = sg_list[curr_sge_idx];
  return {static_cast<char*>(sge.addr) + curr_sge_offset, sge.length - curr_sge_offset};
}

bool WriteTaskContext::update_sent(size_t bytes) {
  if (is_completed()) return false;
  curr_sge_offset += bytes;
  if (curr_sge_offset >= sg_list[curr_sge_idx].length) {
    curr_sge_idx++;
...
ReadTaskContext::ReadTaskContext(const TcpReadWR* wr) {
  if (!wr) return;
  remote_mr = wr->remote_mr;
  for (const auto* curr_wr = wr; curr_wr; curr_wr = curr_wr->next) {
    if (!curr_wr->sg_list) continue;
...
bool ReadTaskContext::is_completed() const {
  return curr_sge_idx >= sg_list.size() && ctrl_sent && data_received;
}

std::vector<char> ReadTaskContext::get_ctrl_header(const Token& token) const {
  TcpCtrlHeader header;
...
std::pair<void*, size_t> ReadTaskContext::get_current_buffer() const {
  if (is_completed()) return {nullptr, 0};
  const auto& sge = sg_list[curr_sge_idx];
  return {static_cast<char*>(sge.addr) + curr_sge_offset, sge.length - curr_sge_offset};
}

bool ReadTaskContext::update_received(size_t bytes) {
  if (is_completed()) return false;
  curr_sge_offset += bytes;
  if (curr_sge_offset >= sg_list[curr_sge_idx].length) {
    curr_sge_idx++;
...
TcpQueuePair::TcpQueuePair(const Token& token) 
  : sock_fd_(-1), state_(TcpQPState::Reset), token_(token) {}

TcpQueuePair::~TcpQueuePair() { 
  if (sock_fd_ != -1) close(sock_fd_); 
}

bool TcpQueuePair::modifyToInit() {
...
bool TcpQueuePair::modifyToRTR(const GID& gid, const TcpGIDMap& gid_map) {
  ...
  TcpCtrlHeader header;
  header.op = TcpCtrlHeader::Op::Token;
  header.token = token_;
  header.gid = gid;
  std::vector<char> ctrl_buf(sizeof(header));
  std::memcpy(ctrl_buf.data(), &header, sizeof(header));
  ssize_t sent_bytes = send(sock_fd_, ctrl_buf.data(), ctrl_buf.size(), MSG_NOSIGNAL);
  if (sent_bytes != static_cast<ssize_t>(ctrl_buf.size())) return false;
...
void TcpQueuePair::postSend(const TcpSendWR* wr) {
  if (state_ != TcpQPState::Rts) throw std::runtime_error("QP not in RTS state");
  std::lock_guard<std::mutex> lock(send_mutex_);
  send_queue_.emplace(wr);
}

void TcpQueuePair::postRecv(const TcpRecvWR* wr) {
  if (state_ != TcpQPState::Rtr) throw std::runtime_error("QP not in RTR state");
  std::lock_guard<std::mutex> lock(recv_mutex_);
  recv_queue_.emplace(wr);
}

void TcpQueuePair::postWrite(const TcpWriteWR* wr) {
  if (state_ != TcpQPState::Rts) throw std::runtime_error("QP not in RTS state");
  std::lock_guard<std::mutex> lock(write_mutex_);
  write_queue_.emplace(wr);
}

void TcpQueuePair::postRead(const TcpReadWR* wr) {
  if (state_ != TcpQPState::Rts) throw std::runtime_error("QP not in RTS state");
  std::lock_guard<std::mutex> lock(read_mutex_);
  read_queue_.emplace(wr);
}

int TcpQueuePair::getSockFd() const { 
  return sock_fd_; 
}

TcpQPState TcpQueuePair::getState() const { 
  return state_; 
}

void TcpQueuePair::setState(TcpQPState state) { 
  state_ = state; 
}

std::queue<SendTaskContext>& TcpQueuePair::getSendQueue() { 
  return send_queue_; 
}

std::mutex& TcpQueuePair::getSendMutex() { 
  return send_mutex_; 
}

std::queue<RecvTaskContext>& TcpQueuePair::getRecvQueue() { 
  return recv_queue_; 
}

std::mutex& TcpQueuePair::getRecvMutex() { 
  return recv_mutex_; 
}

std::queue<WriteTaskContext>& TcpQueuePair::getWriteQueue() { 
  return write_queue_; 
}

std::mutex& TcpQueuePair::getWriteMutex() { 
  return write_mutex_; 
}

std::queue<ReadTaskContext>& TcpQueuePair::getReadQueue() { 
  return read_queue_; 
}

std::mutex& TcpQueuePair::getReadMutex() { 
  return read_mutex_; 
}

const Token& TcpQueuePair::getToken() const { 
  return token_; 
}

void TcpManager::TcpCompletionQueue::add(const TcpWC& wc) {
  std::lock_guard<std::mutex> lock(mutex_);
  queue_.push(wc);
}

int TcpManager::TcpCompletionQueue::poll(TcpWC* wc, int num) {
  std::lock_guard<std::mutex> lock(mutex_);
  int count = 0;
...
TcpManager::ConnectionConfig::ConnectionConfig() = default;
TcpManager::QPConfig::QPConfig() = default;

TcpManager::TcpManager() 
  : epoll_fd_(-1), wakeup_pipe_{-1, -1}, listen_fd_(-1),
    initialized_(false), running_(false), next_conn_id_(1), next_qp_id_(1) {}

TcpManager::~TcpManager() {
  running_ = false;
  if (wakeup_pipe_[1] != -1) {
    ssize_t result = write(wakeup_pipe_[1], "", 1);
    (void)result; // 忽略返回值警告
  }
...
bool TcpManager::initialize(const std::string& local_ip, const Token& token) {
  ...
  try {
    ...
    if (pipe2(wakeup_pipe_, O_NONBLOCK | O_CLOEXEC) == -1) throw std::runtime_error("pipe2 failed");
    
    epoll_event wakeup_ev;
    wakeup_ev.events = EPOLLIN;
    wakeup_ev.data.fd = wakeup_pipe_[0];
    if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, wakeup_pipe_[0], &wakeup_ev) == -1) throw std::runtime_error("epoll_ctl failed");
    ...
    if (bind(listen_fd_, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) == -1) throw std::runtime_error("bind failed");
    if (listen(listen_fd_, SOMAXCONN) == -1) throw std::runtime_error("listen failed");
    
    epoll_event listen_ev;
    listen_ev.events = EPOLLIN;
    listen_ev.data.fd = listen_fd_;
    if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, listen_fd_, &listen_ev) == -1) throw std::runtime_error("epoll_ctl failed");
...
void TcpManager::registerGID(const GID& gid, const std::string& ip, uint16_t port) {
  gid_map_.registerGID(gid, ip, port);
}

TcpManager::ConnectionId TcpManager::createConnection(const ConnectionConfig& config) {
  ...
  connections_.emplace(id, conn_info);
  
  return id;
}

TcpManager::QPId TcpManager::createQP(ConnectionId conn_id, const QPConfig& config) {
  if (!initialized_) return 0;
  std::lock_guard<std::mutex> lock(conn_mutex_);
  ...
  QPId id = next_qp_id_++;
  it->second.qps[id] = std::make_shared<TcpQueuePair>(token_);
  return id;
}

bool TcpManager::modifyQPToInit(ConnectionId conn_id, QPId qp_id) {
  auto qp = getQP(conn_id, qp_id);
  return qp && qp->modifyToInit();
}

bool TcpManager::modifyQPToRTR(ConnectionId conn_id, QPId qp_id, const GID& gid) {
  auto qp = getQP(conn_id, qp_id);
  return qp && qp->modifyToRTR(gid, gid_map_);
}

bool TcpManager::modifyQPToRTS(ConnectionId conn_id, QPId qp_id) {
  auto qp = getQP(conn_id, qp_id);
...
bool TcpManager::postSend(ConnectionId conn_id, QPId qp_id, const TcpSendWR* wr) {
  auto qp = getQP(conn_id, qp_id);
  if (!qp) return false;
  qp->postSend(wr);
  wakeupWorker();
...
bool TcpManager::postRecv(ConnectionId conn_id, QPId qp_id, const TcpRecvWR* wr) {
  auto qp = getQP(conn_id, qp_id);
  if (!qp) return false;
  qp->postRecv(wr);
  wakeupWorker();
...
bool TcpManager::postWrite(ConnectionId conn_id, QPId qp_id, const TcpWriteWR* wr) {
  auto qp = getQP(conn_id, qp_id);
  if (!qp) return false;
  qp->postWrite(wr);
  wakeupWorker();
...
bool TcpManager::postRead(ConnectionId conn_id, QPId qp_id, const TcpReadWR* wr) {
  auto qp = getQP(conn_id, qp_id);
  if (!qp) return false;
  qp->postRead(wr);
  wakeupWorker();
...
int TcpManager::pollCQ(ConnectionId conn_id, int num_entries, TcpWC* wc) {
  std::lock_guard<std::mutex> lock(conn_mutex_);
  auto it = connections_.find(conn_id);
...
void TcpManager::handleNewConnection() {
  ...
  TcpCtrlHeader resp;
  resp.op = TcpCtrlHeader::Op::Token;
  resp.token = token_;
  resp.gid = header.gid;
...
void TcpManager::handleSocketEvent(int fd, uint32_t events) {
...
void TcpManager::handleWriteEvent(int fd, std::shared_ptr<TcpQueuePair> qp, std::shared_ptr<TcpCompletionQueue> cq) {
  ...
  while (!write_queue.empty()) {
    ...
    if (!task.ctrl_sent) {
      ...
      ssize_t sent_bytes = send(fd, ctrl.data(), ctrl.size(), MSG_NOSIGNAL);
      if (sent_bytes != static_cast<ssize_t>(ctrl.size())) break;
      task.ctrl_sent = true;
    }
    auto [data, len] = task.get_current_data();
    ssize_t sent = send(fd, data, len, MSG_NOSIGNAL);
...
void TcpManager::handleReadEvent(int fd, std::shared_ptr<TcpQueuePair> qp, std::shared_ptr<TcpCompletionQueue> cq) {
  ...
  std::memcpy(&header, ctrl_buf.data(), sizeof(header));
...
void TcpManager::handleRemoteWrite(int fd, std::shared_ptr<TcpQueuePair> qp, std::shared_ptr<TcpCompletionQueue> cq, const TcpCtrlHeader& header) {
...
void TcpManager::handleRemoteRead(int fd, std::shared_ptr<TcpQueuePair> qp, std::shared_ptr<TcpCompletionQueue> cq, const TcpCtrlHeader& header) {
  auto mr = pd_->lookUpMR(header.remote_rkey);
  if (!mr) {
...
std::shared_ptr<TcpQueuePair> TcpManager::getQP(ConnectionId conn_id, QPId qp_id) {
  std::lock_guard<std::mutex> lock(conn_mutex_);
  auto it = connections_.find(conn_id);
...
std::shared_ptr<TcpManager::TcpCompletionQueue> TcpManager::getCQ(ConnectionId conn_id) {
  std::lock_guard<std::mutex> lock(conn_mutex_);
  auto it = connections_.find(conn_id);
  if (it == connections_.end()) return nullptr;
  return it->second.cq;
...
void TcpManager::wakeupWorker() {
  char buf[] = "w";
  ssize_t result = write(wakeup_pipe_[1], buf, sizeof(buf));
  (void)result; // 忽略返回值警告
}

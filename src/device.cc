...
//   if (!empty() && flags_ != 0x7f7f7f7f)
//     (void)cudaStreamDestroy(stream_);
// }

// void CudaStreamWithFlags::set(unsigned int flags) {
//   if (!empty())
//     throw std::runtime_error("CudaStreamWithFlags already set");
//   CUDACHECK(cudaStreamCreateWithFlags(&stream_, flags));
// }
...
//   CUDACHECK(cudaMallocAsync(&ptr, bytes, stream));
...
//   (CUmemAllocationHandleType)(requestedHandleTypes); prop.location.id =
//   currentDevice;

//   if (gran == 0) {
//     gran = getMulticastGranularity(bytes,
...
//   if (align == 0) {
//     align = getMulticastGranularity(nbytes,
//     CU_MULTICAST_GRANULARITY_MINIMUM);
//   }

//   void *devicePtr = nullptr;
//   CUCHECK(cuMemAddressReserve((CUdeviceptr *)&devicePtr, nbytes, align, 0U,
//   0)); CUCHECK(cuMemMap((CUdeviceptr)devicePtr, nbytes, 0, memHandle, 0));

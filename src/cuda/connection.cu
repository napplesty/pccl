...
// #include "runtime.h" // For pccl::Transport, pccl::RegisteredMemory,
// pccl::TransportInfo (constructor param)

// // Assuming cuda/registered_memory.h is included via cuda/connection.h for
// the
// // complex RegisteredMemory::Impl #include "cuda/registered_memory.h" //
// Should
...
//     oss << "CUDA error in " << file << " at line " << line << " (" <<
//     call_str
...
//   PCCL_LOG_INFO("CudaIpcConnection destroyed for remoteRank %d",
//   remoteRank_);
// }

// void CudaIpcConnection::switchBackend(void * /*backend*/) {
//   PCCL_LOG_WARN(
//       "CudaIpcConnection::switchBackend is not implemented for remoteRank
//       %d.", remoteRank_);
// }
...
//        << ", Access Size: " << access_size << ", Memory Size: " <<
//        mem.size();
...
//                 static_cast<void *>(dstDevPtr), dstOffset, size,
//                 remoteRank_);

//   PCCL_CUDA_CHECK(cudaMemcpyAsync(dstDevPtr + dstOffset, srcDevPtr +
//   srcOffset,
//                                   size, cudaMemcpyDeviceToDevice,
//                                   cudaStreamDefault));
// }

// void CudaIpcConnection::updateAndSync(RegisteredMemory dst, uint64_t
// dstOffset,
//                                       RegisteredMemory src, uint64_t
//                                       srcOffset, uint64_t size) {
//   validate_pccl_ipc_memory(dst, dstOffset, size, "Destination");
//   validate_pccl_ipc_memory(src, srcOffset, size, "Source");
...
//     PCCL_LOG_ERROR(
//         "get_ib_mr_from_pccl_memory: Failed to get IB MR for transport %d:
//         %s", static_cast<int>(ib_transport_type), e.what());
//     return nullptr;
//   }
// }
...
// std::string extract_ib_device_name(const pccl::TransportInfo
// &conn_info_param) {
...
//   PCCL_LOG_WARN("extract_ib_port: Placeholder, returning 1. Implement based
//   on "
...
// }
// IbQpInfo extract_remote_ib_qp_info(const pccl::TransportInfo
// &conn_info_param) {
...
//     PCCL_LOG_ERROR(
//         "IbConnection Constructor: IB device name is empty for remoteRank
//         %d.", remoteRank_);
//     throw std::runtime_error("IB device name not provided for
//     IbConnection.");
//   }
...
//     qp_ = ctx_->createQp(max_cq_size, max_cq_poll_num, max_send_wr,
//     max_recv_wr,
...
//     PCCL_LOG_INFO("IbConnection created and connected for remoteRank %d on
//     dev "
//                   "%s, port %d. QP num %u.",
...
//       delete qp_;
//       qp_ = nullptr;
//     } // qp_ is owned by IbCtx if created via ctx_->createQp, but if
//     standalone:
//       // delete qp_;
//     if (ctx_) {
//       delete ctx_;
//       ctx_ = nullptr;
//     }
//     PCCL_LOG_ERROR("IbConnection Constructor: Exception for remoteRank %d:
//     %s",
//                    remoteRank_, e.what());
//     throw; // Re-throw
//   }
...
//   // list in ctx_. For now, assume IbCtx destructor cleans up QPs it created.
//   If
//   // createQp returns new IbQp(), then delete qp_; delete ctx_; If IbCtx owns
//   // QPs, then only delete ctx_; Given `std::list<std::unique_ptr<IbQp>>
//   qps;`
//   // in IbCtx, deleting ctx_ should handle it.
//   delete ctx_; // This should trigger unique_ptr destructors for QPs in
//   IbCtx's
//                // list.
//   ctx_ = nullptr;
...
//   // connection context. For simplicity, assuming Transport::IB0. This needs
//   a
//   // proper mechanism.
...
//       dst_mr_info.addr == 0) { // Basic check for uninitialized remote MR
//       info
...
//                                  uint64_t size) {
//   if (!qp_ || !ctx_) {
//     PCCL_LOG_ERROR("IbConnection::updateAndSync: QP or Ctx not initialized
//     for "
//                    "remoteRank %d.",
//                    remoteRank_);
//     throw std::runtime_error(
...
//   if (!src_mr) {
//     PCCL_LOG_ERROR("IbConnection::updateAndSync: Failed to get source IbMr
//     for "
//                    "remoteRank %d.",
//                    remoteRank_);
//     throw std::runtime_error("IbConnection updateAndSync: Source MR is
//     null.");
//   }
...
//   PCCL_LOG_INFO(
//       "IbConnection updateAndSync: Polling for completion for remoteRank
//       %d.", remoteRank_);
...
//         PCCL_LOG_ERROR("IbConnection::updateAndSync: Completion with error
//         for "
...
//   }
//   PCCL_LOG_INFO(
//       "IbConnection flushing connection for remoteRank %d (timeout %lld
//       us).", remoteRank_, (long long)timeoutUsec);

//   // This is a best-effort flush. A true flush often needs to know how many
//   // operations are outstanding. Here, we just poll until the CQ is empty for
//   a
//   // number of tries or timeout.
//   // TODO: Implement proper timeout handling using a timer.
//   int polls = 0;
//   const int max_empty_polls_streak =
//       1000; // If CQ is empty for this many consecutive polls, assume
//       flushed.
...
//   if (!mem.pimpl_) {
//     PCCL_LOG_ERROR(
//         "get_sock_mr_info_from_pccl_memory: RegisteredMemory pimpl_ is
//         null.");
//     return {};
//   }
//   try {
...
// // Placeholder: Extract necessary Socket setup details from
// pccl::TransportInfo
// // (runtime.h)
...
//     // plugin/sock.h indicates SockCtx uses enable_shared_from_this, implies
//     it
...
//     // factory. MSCCLPP's EthernetConnection creates Sockets directly.
//     Assuming
//     // direct construction of SockCtx is okay here, and it internally starts
//     its
...
//     int max_cq_size = pccl::Config::DefaultMaxCqSize;
//     int max_wr_size = pccl::Config::DefaultMaxSendWr; // Assuming MaxSendWr
//     maps
...
//       ctx_ = nullptr;
//       PCCL_LOG_ERROR(
//           "SockConnection Constructor: Failed to create QP for remoteRank
//           %d.", remoteRank_);
...
//       // qp_ is a shared_ptr, will be cleaned up. ctx_ needs manual deletion
//       if
//       // new'd.
//       delete ctx_;
//       ctx_ = nullptr;
...
//   // automatically when this SockConnection object is destroyed, assuming
//   it's
//   // the only owner or last owner. If SockCtx was `new`-ed and is owned
//   solely
//   // by this connection:
//   delete ctx_;
//   ctx_ = nullptr;
//   // qp_ = nullptr; // Not strictly necessary for shared_ptr, but good
//   practice
...
//   if (!qp_ || !ctx_) {
//     PCCL_LOG_ERROR(
//         "SockConnection::write: QP or Ctx not initialized for remoteRank
//         %d.", remoteRank_);
//     throw std::runtime_error(
...
//   if (!src_mr) {
//     PCCL_LOG_ERROR(
//         "SockConnection::write: Failed to get source SockMr for remoteRank
//         %d.", remoteRank_);
...
//   // TODO: Manage wr_id properly. A static counter is not safe for
//   concurrency.
//   // It should ideally be part of the SockQp or a connection-specific
//   sequence. static uint64_t next_sock_wr_id = 1; uint64_t current_wr_id =
//   next_sock_wr_id++;
...
//   if (!src_mr) {
//     PCCL_LOG_ERROR("SockConnection::updateAndSync: Failed to get source
//     SockMr "
//                    "for remoteRank %d.",
//                    remoteRank_);
//     throw std::runtime_error(
...
//   PCCL_LOG_INFO("SockConnection updateAndSync: size %lu to remoteRank %d,
//   wrId "
...
//   PCCL_LOG_INFO("SockConnection updateAndSync: Polling for wrId %lu
//   completion "
//                 "for remoteRank %d.",
...
//     }
//     for (int i = 0; i < num_wcs; ++i) {
//       // pccl::SockQp::getWcStatus(idx) returns SockWc by value in
//       plugin/sock.h SockWc wc = qp_->getWcStatus(i); if (wc.wr_id ==
//       current_wr_id) {
...
//                          remoteRank_);
//           // throw std::runtime_error("Socket operation completed with
//           error.");
//         } else {
...
//   if (!qp_ || !ctx_) {
//     PCCL_LOG_ERROR(
//         "SockConnection::flush: QP or Ctx not initialized for remoteRank
//         %d.", remoteRank_);
//     return;
//   }
//   PCCL_LOG_INFO(
//       "SockConnection flushing connection for remoteRank %d (timeout %lld
//       us).", remoteRank_, (long long)timeoutUsec);
...
//     int num_wcs = qp_->pollCq();
//     if (num_wcs < 0) {
//       PCCL_LOG_ERROR("SockConnection::flush: pollCq error for remoteRank
//       %d.",
//                      remoteRank_);
//       break;
//     }

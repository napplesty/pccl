...
//       transportInfo.cudaIpcOffsetFromBase = (char *)data - (char
//       *)baseDataPtr;
//     }
...
//     pimpl_ = std::make_shared<Impl>(buffer.data(),
//     Config::MAX_LIB_BUFFER_SIZE,
...
//   std::copy_n(reinterpret_cast<char *>(&transportCount),
//   sizeof(transportCount),
//               std::back_inserter(result));
...
//                     reinterpret_cast<char
//                     *>(&transportInfo.cudaIpcBaseHandle));
...
//     // The memory is local to the machine but not to the process, so we need
//     to
...
//       CUmemGenericAllocationHandle handle;
//       if (getNvlsMemHandleType() == CU_MEM_HANDLE_TYPE_FABRIC) {
//         CUCHECK(cuMemImportFromShareableHandle(&handle,
//         entry.shareableHandle,
...
//           getMulticastGranularity(size,
//           CU_MULTICAST_GRANULARITY_RECOMMENDED);
//       size_t size = (this->size + recommendedGran - 1) / recommendedGran *
//                     recommendedGran;
//       CUCHECK(cuMemAddressReserve((CUdeviceptr *)&base, size, minGran, 0,
//       0)); CUCHECK(cuMemMap((CUdeviceptr)base, size, 0, handle, 0));
//       setRWAccess(base, size);
//       this->data = static_cast<char *>(base) + entry.offsetFromBase;
// #else
//       throw std::runtime_error("CUDA does not support NVLS. Please ensure
//       your "
//                                "CUDA version supports NVLS to use this "
//                                "feature.");
// #endif
